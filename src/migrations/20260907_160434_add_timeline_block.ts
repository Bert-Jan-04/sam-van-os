...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TABLE "pages_blocks_timeline_phases" (
  ...
  CREATE TABLE "pages_blocks_timeline" (
  	...
  	"id" varchar PRIMARY KEY NOT NULL,
  	"heading" varchar,
  	"closing_statement" varchar,
  	"block_name" varchar
  );

  CREATE TABLE "_pages_v_blocks_timeline_phases" (
  	"_order" integer NOT NULL,
  	"_parent_id" integer NOT NULL,
  ...
  ALTER TABLE "pages_blocks_timeline_phases" ADD CONSTRAINT "pages_blocks_timeline_phases_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."pages_blocks_timeline"("id") ON DELETE cascade ON UPDATE no action;
  ...
  ALTER TABLE "_pages_v_blocks_timeline_phases" ADD CONSTRAINT "_pages_v_blocks_timeline_phases_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."_pages_v_blocks_timeline"("id") ON DELETE cascade ON UPDATE no action;
  ...
  CREATE INDEX "pages_blocks_timeline_phases_order_idx" ON "pages_blocks_timeline_phases" USING btree ("_order");
  CREATE INDEX "pages_blocks_timeline_phases_parent_id_idx" ON "pages_blocks_timeline_phases" USING btree ("_parent_id");
  ...
  CREATE INDEX "_pages_v_blocks_timeline_phases_order_idx" ON "_pages_v_blocks_timeline_phases" USING btree ("_order");
  CREATE INDEX "_pages_v_blocks_timeline_phases_parent_id_idx" ON "_pages_v_blocks_timeline_phases" USING btree ("_parent_id");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
   DROP TABLE "pages_blocks_timeline_phases" CASCADE;
  DROP TABLE "pages_blocks_timeline" CASCADE;
  DROP TABLE "_pages_v_blocks_timeline_phases" CASCADE;
